...
export function drawTexture(gl, texture, dims, flipHorizontal, rotation) {
    const { program, vao, vertices, uniformLocations } = drawTextureProgram(gl, flipHorizontal, rotation);
...
export function runResizeProgram(gl, inputTexture, inputDims, outputDims, alignCorners, interpolation, rotation) {
    const debugMode = getDebugMode();
    const { program, vao, vertices, uniformLocations } = resizeProgram(gl, inputDims, outputDims, alignCorners, interpolation, rotation);
    gl.useProgram(program);
    // Set up geometry
    webgl_util.callAndCheck(gl, debugMode, () => {
        gl.bindVertexArray(vao);
...
function drawTextureProgram(gl, flipHorizontal, rotation) {
    ...
    if (!programCache.has(cacheKey)) {
        const vertSource = drawTextureProgramInfo.vertexShaderSource(flipHorizontal, rotation);
...
function resizeProgram(gl, sourceDims, targetDims, alignCorners, interpolation, rotation) {
    if (!programCacheByContext.has(gl)) {
        programCacheByContext.set(gl, new Map());
    }
    const programCache = programCacheByContext.get(gl);
